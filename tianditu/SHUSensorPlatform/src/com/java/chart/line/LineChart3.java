...
public class LineChart3 {
	public static String genLineChart(HttpSession session) throws Exception {
		...
		TimeSeries timeSeries =new TimeSeries("风向",Day.class);
		...
		ResultSet rt = st.executeQuery("select numeric_value from observation "
				...
				+ "and phenomenon_id='urn:liesmars:def:phenomenon:LIESMARS:1.0.0:WindDirection'");
		int i=0;
		 while(rt.next())
		 { 
...
//		timeSeries.add(new Month(12,2013),300);
		...
		JFreeChart chart=ChartFactory.createTimeSeriesChart("最近的观测数据", "",".", lineDataset, true, true, true);
